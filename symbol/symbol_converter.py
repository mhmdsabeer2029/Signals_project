    ...
    def tokens_to_events(self, tokens: List[lz77Token]) -> List[DEFLATEEvent]:
        ...
        for token in tokens:
            if isinstance(token, Literal):
                ...
                events.append(MatchEvent(len_sym, len_extra, dist_sym, dist_extra))
    ...
    def events_to_tokens(self, events: List[DEFLATEEvent]) -> List[lz77Token]:
        ...
        for event in events:
            if isinstance(event, LiteralEvent):
                ...
                length = self.length_coder.decode(
                    event.length_symbol, event.length_extra
                )
                distance = self.distance_coder.decode(
                    event.distance_symbol, event.distance_extra
                )
...
def tokens__events(tokens: List[lz77Token]) -> List[DEFLATEEvent]:
    return SymbolConverter().tokens_to_events(tokens)
