...
from symbol.deflate_events import *
            ...
            if isinstance(event, LiteralEvent):
                ...
                #evaluate matchToken distance and lenght values
                    #step1: get extra bits values
                        ...
                        if len(event.length_extra) ==0  : lengthExtraBitsValue = 0
                        else: lengthExtraBitsValue   = int(event.length_extra, 2)
                        if len(event.distance_extra) ==0: distanceExtraBitsValue =0
                        else: distanceExtraBitsValue = int(event.distance_extra, 2)
                    #step2: get index of base value in the bases arrays
                        lengthBaseIdx   = event.length_symbol - 257
                        distacneBaseIdx = event.distance_symbol - 0
...
#made for the interface
def decopressor_fun(events: list[DEFLATEEvent]) -> list[lz77Token]:
    obj = s2Decompressor(events)
    tokensStream= obj.decopress()
    return tokensStream